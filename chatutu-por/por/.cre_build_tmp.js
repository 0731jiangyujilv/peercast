...
var getOffchainData = (runtime2) => {
  runtime2.log(`\uD83D\uDCE1 Fetching offchain data from ${runtime2.config.botApiUrl}`);
  const httpClient = new ClientCapability2;
  const statsResponse = httpClient.sendRequest(runtime2, {
    method: "GET",
    url: `${runtime2.config.botApiUrl}/api/stats`
  }).result();
  if (statsResponse.statusCode !== 200) {
    throw new Error(`Failed to fetch stats: ${statsResponse.statusCode}`);
  }
  const leaderboardResponse = httpClient.sendRequest(runtime2, {
    method: "GET",
    url: `${runtime2.config.botApiUrl}/api/stats/leaderboard?limit=10`
  }).result();
  if (leaderboardResponse.statusCode !== 200) {
    throw new Error(`Failed to fetch leaderboard: ${leaderboardResponse.statusCode}`);
  }
  const stats = JSON.parse(Buffer.from(statsResponse.body).toString("utf-8"));
  const leaderboard = JSON.parse(Buffer.from(leaderboardResponse.body).toString("utf-8"));
  const data = {
    stats,
    leaderboard,
    timestamp: Date.now()
  };
  runtime2.log(`\uD83D\uDCCA Offchain Stats: ${JSON.stringify(data.stats)}`);
  runtime2.log(`\uD83C\uDFC6 Leaderboard entries: ${data.leaderboard.length}`);
  return data;
};
var getOnchainData = (runtime2) => {
  runtime2.log("⛓️  Reading onchain data");
  ...
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: evmConfig.betFactoryAddress,
      data: callData
    })
  }).result();
  ...
  runtime2.log(`\uD83D\uDCC8 Onchain total bets: ${totalBets.toString()}`);
  return {
    totalBetsOnChain: totalBets,
    activeBetsOnChain: 0n
  };
};
var verifyData = (runtime2, offchain, onchain) => {
  runtime2.log("\uD83D\uDD0D Verifying data consistency");
  const offchainTotal = BigInt(offchain.stats.totalBetsCount);
  const onchainTotal = onchain.totalBetsOnChain;
  const isValid2 = offchainTotal === onchainTotal;
  if (!isValid2) {
    runtime2.log(`⚠️  Data mismatch: Offchain=${offchainTotal}, Onchain=${onchainTotal}`);
  } else {
    runtime2.log("✅ Data verification passed");
  }
  return isValid2;
};
var updatePoRContract = (runtime2, offchain, onchain, isValid2) => {
  runtime2.log("\uD83D\uDCDD Updating PoR contract");
  const evmConfig = runtime2.config.evms[0];
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: evmConfig.chainSelectorName,
  ...
  const topPlayerProfit = topPlayer ? BigInt(Math.floor(parseFloat(topPlayer.totalProfit) * 1000000000000000000)) : 0n;
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 totalBets, uint256 activeBets, uint256 settledBets, uint256 totalVolume, uint256 topPlayerProfit, bool isValid, uint256 timestamp"), [
    BigInt(offchain.stats.totalBetsCount),
    BigInt(offchain.stats.activeBetsCount),
    BigInt(offchain.stats.settledBetsCount),
    BigInt(Math.floor(parseFloat(offchain.stats.totalVolume) * 1000000000000000000)),
    topPlayerProfit,
    isValid2,
  ...
  runtime2.log(`✅ PoR contract updated: ${txHashHex}`);
  return txHashHex;
};
